      ...
      <section className="panel rounded-2xl p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div className="flex items-start gap-4">
            ...
            <div>
            <p className="text-sm font-black uppercase text-yellow-300">Ranking</p>
            <h1 className="mt-2 flex items-center gap-2 text-3xl font-black text-white"><Trophy size={30} className="text-yellow-400" /> Leaderboard</h1>
            </div>
          </div>
          <p className="text-sm font-semibold text-slate-400">Diurutkan berdasarkan winrate, lalu total win.</p>
        </div>
        ...
        <div className="mt-6 overflow-x-auto rounded-2xl border border-slate-700 bg-slate-900/70">
          <table className="w-full min-w-[720px] border-collapse text-left text-sm">
            ...
            <tbody>
              {loading && (
                <tr><td className="px-4 py-5 text-center font-semibold text-slate-500" colSpan="6">Loading...</td></tr>
              )}
              {!loading && users.length === 0 && (
                <tr><td className="px-4 py-5 text-center font-semibold text-slate-500" colSpan="6">Belum ada data leaderboard.</td></tr>
              )}
              {users.map((user, index) => (
                <tr key={user._id} className="border-t border-slate-700 text-slate-300 transition hover:bg-white/5">
                  <td className="px-4 py-3 font-black text-yellow-300">
                    {index < 3 ? <Crown size={18} className="inline" /> : `#${index + 1}`}
                  </td>
